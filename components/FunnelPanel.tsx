...
function DroppableFunnel({
  funnel, expanded, onToggle, onDelete, onRemoveItem, activeDragId,
}: {
  funnel: Funnel;
  expanded: boolean;
  onToggle: () => void;
  ...
  return (
    ...
    >
      <button onClick={onToggle} className="flex items-center justify-between gap-2 group">
              ...
              funnel.items.map((item) => (
                ...
                >
                  <div className="flex-1 min-w-0">
                    <p className="text-[10px] font-medium text-slate-300 truncate">{item.result.name}</p>
                    {item.result.email && (
...
// ── FunnelPanel (slide-in drawer) ───────────────────────────────────────────

export default function FunnelPanel({
  funnels, onFunnelsChange, onAddToFunnel, onRemoveFromFunnel,
  topOffset = "top-14", activeDragId,
}: Props) {
  ...
  // Auto-open when dragging a card
  const shouldShow = open || !!activeDragId;
  ...
  const handleCreateFunnel = async () => {
    ...
    try {
      ...
      setNewName(""); setNewType("custom"); setNewDescription(""); setShowNewForm(false);
  ...
  const totalLeads = funnels.reduce((s, f) => s + f.items.length, 0);
  ...
  return (
    <>
      ...
      <button
        ...
        className={`fixed right-0 ${topOffset} z-30 flex items-center gap-1.5
                    pl-2.5 pr-2 py-3 rounded-l-xl
                    bg-slate-900/95 border border-r-0 border-slate-700/60
                    text-slate-400 hover:text-sky-400 hover:bg-slate-800/95
                    shadow-lg shadow-black/30 backdrop-blur-sm
                    transition-all duration-200
                    ${shouldShow ? "translate-x-full opacity-0 pointer-events-none" : "translate-x-0 opacity-100"}`}
        style={{ marginTop: "1rem" }}
      >
        <Anchor className="w-3.5 h-3.5" />
        <span className="text-[10px] font-bold tracking-wider uppercase writing-mode-vertical"
              style={{ writingMode: "vertical-rl", textOrientation: "mixed", letterSpacing: "0.1em" }}>
          Funnels
        </span>
        {totalLeads > 0 && (
          <span className="absolute -left-1 -top-1 w-4 h-4 rounded-full bg-sky-500 text-[9px] text-white font-bold flex items-center justify-center">
      ...
      <AnimatePresence>
        {shouldShow && (
          <motion.div
            ...
            className="fixed inset-0 bg-black/40 backdrop-blur-[2px] z-30 sm:bg-black/20"
          />
        )}
      </AnimatePresence>
      ...
      <motion.div
        ...
        className={`fixed right-0 ${topOffset} ${heightClass} w-72 bg-slate-900/95 backdrop-blur-sm border-l border-slate-700/60 z-40 flex flex-col shadow-2xl shadow-black/40`}
      >
        {/* Header */}
        <div className="px-3 py-2.5 border-b border-slate-700/60 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[10px] text-slate-600">{funnels.length}</span>
          </div>
          <div className="flex items-center gap-1">
            <button onClick={() => setShowNewForm(!showNewForm)}
              className="p-1 rounded-lg hover:bg-sky-500/10 text-sky-400 transition-colors" title="New funnel">
              <Plus className="w-3.5 h-3.5" />
            </button>
            <button onClick={() => router.push("/funnels")}
              className="p-1 rounded-lg hover:bg-slate-700/60 text-slate-500 hover:text-slate-300 transition-colors" title="Full page">
              <Maximize2 className="w-3.5 h-3.5" />
            </button>
            <button onClick={() => setOpen(false)}
              className="p-1 rounded-lg hover:bg-slate-700/60 text-slate-500 hover:text-slate-300 transition-colors" title="Close">
          ...
          {showNewForm && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }}
              ...
              <Input value={newName} onChange={(e) => setNewName(e.target.value)}
                placeholder="Funnel name" className="h-7 text-xs bg-slate-800/60 border-slate-600/60"
                onKeyDown={(e) => e.key === "Enter" && handleCreateFunnel()} />
              <Select value={newType} onValueChange={setNewType}>
                <SelectTrigger className="h-7 text-xs bg-slate-800/60 border-slate-600/60"><SelectValue /></SelectTrigger>
                <SelectContent className="bg-slate-900 border-slate-700">
                  {FUNNEL_TYPES.map((t) => <SelectItem key={t.value} value={t.value} className="text-xs">{t.label}</SelectItem>)}
                </SelectContent>
              </Select>
